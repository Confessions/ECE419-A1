...
public class MazeImpl extends Maze implements Serializable, ClientListener, Runnable {
        ...
        //added helper fucntion to conplete the bullet destory motion
        public synchronized void clientPrjDes(Client client, Point point) 
        {
            assert(client != null);
            ...
            synchronized(projectileMap) {
                    while(it.hasNext()) {   
                            ...
                            Projectile projectileO = (Projectile) o;
                            Client owner = (Client)projectileO.getOwner();
                            if (owner.getName() == client.getName())
                            {
                                /*Object newo = projectileMap.get(projectileO);
                                assert(newo instanceof DirectedPoint);
                                DirectedPoint dp = (DirectedPoint)newo;
            ...
            if(Debug.debug) System.out.println("about to clear "+point.getX()+"," +point.getY());
            CellImpl cell = getCellImpl(point);
            cell.setContents(null);
            update();
        }
        ...
        public void run() {
                ...
                if(Debug.debug) System.out.println("hello from mazeimpl");
                while(true) {
                ...
                Object contents = newCell.getContents();
                     ...
                     if(contents != null) {
                        ...
                        /* Bullets destroy each other
                                ...
                                newCell.setContents(null);//inconsistant
                                cell.setContents(null);
                                ...
                                //instructing new calls that will put two packet on the eventqueue to tell all of the 
                                //clients to destory the projectile in inside the cell
                                if(Debug.debug) System.out.println("in destory");
                                assert(contents instanceof Projectile);
                                Client prjclient1 = prj.getOwner();
                                if(Debug.debug) System.out.println("about to send packet");
                                Point location1 = new Point(dp.getX(),dp.getY());
                                MPacket destory1 = new MPacket(prjclient1.getName(), MPacket.ACTION, MPacket.DESTORY,location1);
                                putQueue(destory1);
                                Projectile prj2 = (Projectile)contents;
                                Client prjclient2 = prj2.getOwner();
                                 Point location2 = new Point(newPoint.getX(),newPoint.getY());
                                MPacket destory2 = new MPacket(prjclient2.getName(), MPacket.ACTION, MPacket.DESTORY,location2);
                                putQueue(destory2);
                                deadPrj.add(prj);
                                deadPrj.add(contents);
                                changeInt(1);
                                if(Debug.debug) System.out.println("about to remove two projectile");
                                return deadPrj;

                        }
                    }

                    if(Debug.debug) System.out.println("Not in destory, passing through");
                    /* Clear the old cell */
                    cell.setContents(null);
                    /* Write the new cell */
                    projectileMap.put(prj, newPoint);
        ...
        /*
        * add the bullet destory action on the queue
        
        public void putQueue (MPacket packet)
        {
